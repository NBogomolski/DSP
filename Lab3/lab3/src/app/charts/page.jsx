...
function Charts() {
  ...
  let secondFuncYCoordinates = calculateFourierCoordinatesSinus(
    samplingFrequency,
    secondFuncFrequency,
    secondFuncPhase0,
    secondFuncAmplitude,
  ...
  const customPolyharmonicYCoordinates = sinYCoordinates.map(
  ...
  if (isFourierTransformed) {
    ...
    transformedData = calcFourier(
      customPolyharmonicYCoordinates,
      samplingFrequency
    );
  ...
  fourierTransformedCustomPolyharmonic =
    fourierTransformedCustomPolyharmonic.map((v, ind) => {
      return { y: v, n: ind + 1 };
    });
  customPolyharmonicAmpSpectrum = customPolyharmonicAmpSpectrum.map(
    (v, ind) => {
      return { y: v, n: ind + 1 };
    }
  );
  customPolyharmonicPhaseSpectrum = customPolyharmonicPhaseSpectrum.map(
    (v, ind) => {
      return { y: v, n: ind + 1 };
    }
  );
  ...
  customPolyharmonic = customPolyharmonic.map((value, ind) => {
    return {
      ...value,
      sin: sinusData[ind].y,
      secondFunc: secondFuncSinus[ind].y,
    };
  });
  ...
  if (isFourierTransformed) {
    ...
    customPolyharmonic = customPolyharmonic.map((v, ind) => {
      return {
        ...v,
        fourierTransformed: fourierTransformedCustomPolyharmonic[ind].y,
      };
    });
  }
  // if (secondFunc) {
